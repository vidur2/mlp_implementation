...
const HIGHER_LEVEL_NUERON_ID: &str = "mlp3.perceptron.testnet";

// Lower Level Nueron Id
const LOWER_LEVEL_NUERON_ID: &str = "mlp1.perceptron.testnet";
...
impl InputNueron2{
    ...
    pub fn predict(&self, input1: f32, input2: f32, input3: f32, input4: f32, input5: f32, mut outputs: Vec<f32>, mut input_vector: Vec<Vec<f32>>, expected_ouput: f32){
        ...
        higher_level_nueron::predict(input_vector[1][0], input_vector[1][1], input_vector, expected_ouput, higher_level_nueron_account_id, NO_DEPOSIT, gas_count);
    }
    pub fn adjust(&mut self, offset: f32, input1: f32, input2: f32, input3: f32, input4: f32, input5: f32){
        ...
        if split_pred_id[split_pred_id.len() - 2] == String::from("perceptron"){
            ...
            let lower_level_nueron_account_id: AccountId = LOWER_LEVEL_NUERON_ID.to_string().trim().parse().expect("invalid");
            let gas_count = Gas::from(BASE_GAS);
            lower_level_nueron::adjust(offset, input1, input2, input3, input4, input5, lower_level_nueron_account_id, NO_DEPOSIT, gas_count);
        }
    }
